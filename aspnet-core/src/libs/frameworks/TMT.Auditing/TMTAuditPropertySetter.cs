using System;
using TMT.Security.Users;
using Volo.Abp;
using Volo.Abp.Auditing;
using Volo.Abp.DependencyInjection;
using Volo.Abp.MultiTenancy;
using Volo.Abp.Timing;
using Volo.Abp.Users;

namespace TMT.Auditing
{
    public class TMTAuditPropertySetter : IAuditPropertySetter, ITransientDependency
    {
        protected ICurrentUser CurrentUser { get; }
        protected ITMTCurrentUser TMTCurrentUser { get; }

        protected ICurrentTenant CurrentTenant { get; }
        protected IClock Clock { get; }

        public TMTAuditPropertySetter(
            ICurrentUser currentUser,
            ICurrentTenant currentTenant,
            IClock clock,
            ITMTCurrentUser tMTCurrentUser)
        {
            CurrentUser = currentUser;
            CurrentTenant = currentTenant;
            Clock = clock;
            TMTCurrentUser = tMTCurrentUser;
        }

        public void SetCreationProperties(object targetObject)
        {
            SetCreationTime(targetObject);
            TMTSetCreatorId(targetObject);
        }

        public void SetModificationProperties(object targetObject)
        {
            SetLastModificationTime(targetObject);
            TMTSetLastModifierId(targetObject);
        }

        public void SetDeletionProperties(object targetObject)
        {
            SetDeletionTime(targetObject);
            TMTSetDeleterId(targetObject);
        }

        private void SetCreationTime(object targetObject)
        {
            if (!(targetObject is IHasCreationTime objectWithCreationTime))
            {
                return;
            }

            if (objectWithCreationTime.CreationTime == default)
            {
                ObjectHelper.TrySetProperty(objectWithCreationTime, x => x.CreationTime, () => Clock.Now);
            }
        }

        private void TMTSetCreatorId(object targetObject)
        {
            if (String.IsNullOrEmpty(TMTCurrentUser.Id))
            {
                return;
            }

            if (targetObject is IMultiTenant multiTenantEntity)
            {
                if (multiTenantEntity.TenantId != TMTCurrentUser.TenantId)
                {
                    return;
                }
            }

            /* TODO: The code below is from old ABP, not implemented yet
                if (tenantId.HasValue && MultiTenancyHelper.IsHostEntity(entity))
                {
                    //Tenant user created a host entity
                    return;
                }
                 */

            if (targetObject is ITMTMayHaveCreator mayHaveCreatorObject)
            {
                //if ( mayHaveCreatorObject.CreatorId.Value != default)
                //{
                //    return;
                //}

                ObjectHelper.TrySetProperty(mayHaveCreatorObject, x => x.CreatorId, () => TMTCurrentUser.Id);
            }
            else if (targetObject is ITMTMustHaveCreator mustHaveCreatorObject)
            {
                //if (mustHaveCreatorObject.CreatorId != default)
                //{
                //    return;
                //}

                ObjectHelper.TrySetProperty(mustHaveCreatorObject, x => x.CreatorId, () => TMTCurrentUser.Id);
            }
        }

        private void SetLastModificationTime(object targetObject)
        {
            if (targetObject is IHasModificationTime objectWithModificationTime)
            {
                objectWithModificationTime.LastModificationTime = Clock.Now;
            }
        }

        private void TMTSetLastModifierId(object targetObject)
        {
            //if (!(targetObject is IModificationAuditedObject modificationAuditedObject))
            //{
            //    return;
            //}
            if (!(targetObject is ITMTModificationAuditedObject modificationAuditedObject))
            {
                return;
            }

            //if (!CurrentUser.Id.HasValue)
            //{
            //    modificationAuditedObject.LastModifierId = null;
            //    return;
            //}

            if (modificationAuditedObject is IMultiTenant multiTenantEntity)
            {
                if (multiTenantEntity.TenantId != CurrentUser.TenantId)
                {
                    modificationAuditedObject.LastModifierId = null;
                    return;
                }
            }

            /* TODO: The code below is from old ABP, not implemented yet
            if (tenantId.HasValue && MultiTenancyHelper.IsHostEntity(entity))
            {
                //Tenant user modified a host entity
                modificationAuditedObject.LastModifierId = null;
                return;
            }
             */

            modificationAuditedObject.LastModifierId = TMTCurrentUser.Id;
        }

        private void SetDeletionTime(object targetObject)
        {
            if (targetObject is IHasDeletionTime objectWithDeletionTime)
            {
                if (objectWithDeletionTime.DeletionTime == null)
                {
                    objectWithDeletionTime.DeletionTime = Clock.Now;
                }
            }
        }

        private void TMTSetDeleterId(object targetObject)
        {
            if (!(targetObject is ITMTDeletionAuditedObject deletionAuditedObject))
            {
                return;
            }

            if (deletionAuditedObject.DeleterId != null)
            {
                return;
            }

            //if (!CurrentUser.Id.HasValue)
            //{
            //    deletionAuditedObject.DeleterId = null;
            //    return;
            //}

            if (string.IsNullOrEmpty(TMTCurrentUser.Id))
            {
                deletionAuditedObject.DeleterId = null;
                return;
            }
            if (deletionAuditedObject is IMultiTenant multiTenantEntity)
            {
                if (multiTenantEntity.TenantId != CurrentUser.TenantId)
                {
                    deletionAuditedObject.DeleterId = null;
                    return;
                }
            }

            deletionAuditedObject.DeleterId = TMTCurrentUser.Id;
        }
    }
}